...
public class PopupView extends LinearLayout {
	...
	public PopupView(Context context) {
		...
		addButtonOptionsToPopup(supported_flash_values, R.array.flash_icons,
				R.array.flash_values,
				getResources().getString(R.string.flash_mode),
				preview.getCurrentFlashValue(), "TEST_FLASH",
		...
		if (preview.isVideo() && preview.isTakingPhoto()) {
			...
			List<String> supported_focus_values = preview
					.getSupportedFocusValues();
			addButtonOptionsToPopup(supported_focus_values,
					R.array.focus_mode_icons, R.array.focus_mode_values,
					getResources().getString(R.string.focus_mode),
					preview.getCurrentFocusValue(), "TEST_FOCUS",
					new ButtonOptionsPopupListener() {
						@Override
						public void onClick(String option) {
							if (MyDebug.LOG)
			...
			String current_iso = sharedPreferences.getString(
					MainActivity.getISOPreferenceKey(), "auto");
			// n.b., we hardcode the string "ISO" as we don't want it translated
			// - firstly more consistent with the ISO values returned by the
			// driver, secondly need to worry about the size of the buttons, so
			// don't want risk of a translated string being too long
			addButtonOptionsToPopup(supported_isos, -1, -1, "ISO", current_iso,
					"TEST_ISO", new ButtonOptionsPopupListener() {
						@Override
						public void onClick(String option) {
							if (MyDebug.LOG)
							...
							SharedPreferences.Editor editor = sharedPreferences
									.edit();
							editor.putString(
									MainActivity.getISOPreferenceKey(), option);
							editor.apply();
			...
			List<String> supported_white_balances = preview
					.getSupportedWhiteBalances();
			addRadioOptionsToPopup(supported_white_balances, getResources()
					.getString(R.string.white_balance),
					MainActivity.getWhiteBalancePreferenceKey(),
					Camera.Parameters.WHITE_BALANCE_AUTO, "TEST_WHITE_BALANCE");

			List<String> supported_scene_modes = preview
					.getSupportedSceneModes();
			addRadioOptionsToPopup(supported_scene_modes, getResources()
					.getString(R.string.scene_mode),
					MainActivity.getSceneModePreferenceKey(),
					Camera.Parameters.SCENE_MODE_AUTO, "TEST_SCENE_MODE");

			List<String> supported_color_effects = preview
					.getSupportedColorEffects();
			addRadioOptionsToPopup(supported_color_effects, getResources()
					.getString(R.string.color_effect),
					MainActivity.getColorEffectPreferenceKey(),
					Camera.Parameters.EFFECT_NONE, "TEST_COLOR_EFFECT");
				...
				checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					public void onCheckedChanged(CompoundButton buttonView,
						...
						SharedPreferences.Editor editor = sharedPreferences
								.edit();
						editor.putBoolean(
								MainActivity.getAutoStabilisePreferenceKey(),
								isChecked);
						editor.apply();
						...
						String message = getResources().getString(
								...
								+ getResources().getString(
										isChecked ? R.string.on : R.string.off);
						preview.showToast(
								main_activity.changed_auto_stabilise_toast,
								message);
						main_activity.closePopup();
					}
				});
			...
			final List<CameraController.Size> picture_sizes = preview
			...
			for (CameraController.Size picture_size : picture_sizes) {
				String size_string = picture_size.width + " x "
						+ picture_size.height;
				picture_size_strings.add(size_string);
			}
			addArrayOptionsToPopup(picture_size_strings, getResources()
					.getString(R.string.preference_resolution),
					picture_size_index, new ArrayOptionsPopupListener() {
						private void update() {
							if (picture_size_index == -1)
								return;
							CameraController.Size new_size = picture_sizes
									.get(picture_size_index);
							String resolution_string = new_size.width + " "
									+ new_size.height;
							SharedPreferences sharedPreferences = PreferenceManager
									.getDefaultSharedPreferences(main_activity);
							SharedPreferences.Editor editor = sharedPreferences
									.edit();
							editor.putString(MainActivity
									.getResolutionPreferenceKey(preview
											.getCameraId()), resolution_string);
							editor.apply();
						...
						public int onClickNext() {
							if (picture_size_index != -1
									&& picture_size_index < picture_sizes
											.size() - 1) {
								picture_size_index++;
								update();
								return picture_size_index;
			...
			final String[] timer_values = getResources().getStringArray(
					R.array.preference_timer_values);
			String[] timer_entries = getResources().getStringArray(
			...
			if (timer_index == -1) {
				if (MyDebug.LOG)
					Log.d(TAG, "can't find timer_value " + timer_value
							+ " in timer_values!");
				timer_index = 0;
			}
			addArrayOptionsToPopup(Arrays.asList(timer_entries), getResources()
					.getString(R.string.preference_timer), timer_index,
					new ArrayOptionsPopupListener() {
						private void update() {
							...
							SharedPreferences.Editor editor = sharedPreferences
									.edit();
							editor.putString(
									MainActivity.getTimerPreferenceKey(),
									new_timer_value);
							editor.apply();
						}
						...
						public int onClickNext() {
							if (timer_index != -1
									&& timer_index < timer_values.length - 1) {
								timer_index++;
								update();
								return timer_index;
							}
	...
	private void addButtonOptionsToPopup(List<String> supported_options,
			int icons_id, int values_id, String string, String current_value,
			String test_key, final ButtonOptionsPopupListener listener) {
		if (MyDebug.LOG)
		...
		if (supported_options != null) {
			...
			if (MyDebug.LOG)
				Log.d(TAG,
						"addButtonOptionsToPopup time 1: "
								+ (System.currentTimeMillis() - time_s));
			String[] icons = icons_id != -1 ? getResources().getStringArray(
					icons_id) : null;
			String[] values = values_id != -1 ? getResources().getStringArray(
					values_id) : null;
			if (MyDebug.LOG)
				Log.d(TAG,
						"addButtonOptionsToPopup time 2: "
								+ (System.currentTimeMillis() - time_s));
			for (final String supported_option : supported_options) {
				...
				if (icons != null && values != null) {
					...
					if (index != -1) {
						resource = getResources().getIdentifier(
								icons[index],
								null,
				...
				if (MyDebug.LOG)
					Log.d(TAG,
							"addButtonOptionsToPopup time 2.1: "
									+ (System.currentTimeMillis() - time_s));
				...
				if (resource != -1) {
					ImageButton image_button = new ImageButton(
							this.getContext());
					if (MyDebug.LOG)
						Log.d(TAG, "addButtonOptionsToPopup time 2.11: "
								+ (System.currentTimeMillis() - time_s));
					view = image_button;
					ll2.addView(view);
					if (MyDebug.LOG)
						Log.d(TAG, "addButtonOptionsToPopup time 2.12: "
								+ (System.currentTimeMillis() - time_s));

					// image_button.setImageResource(resource);
					final MainActivity main_activity = (MainActivity) this
							.getContext();
					...
					if (bm != null)
						...
						if (MyDebug.LOG)
							Log.d(TAG, "failed to find bitmap for resource "
									+ resource + "!");
					}
					if (MyDebug.LOG)
						Log.d(TAG, "addButtonOptionsToPopup time 2.13: "
								+ (System.currentTimeMillis() - time_s));
					image_button.setScaleType(ScaleType.FIT_CENTER);
					final int padding = (int) (10 * scale + 0.5f); // convert
																	// dps to
																	// pixels
					...
					if (string.equalsIgnoreCase("ISO")
							...
							&& supported_option.substring(0, 4)
									.equalsIgnoreCase("ISO_")) {
					...
					// need 0 padding so we have enough room to display text for
					// ISO buttons, when there are 6 ISO settings
					final int padding = (int) (0 * scale + 0.5f); // convert dps
																	// to pixels
					view.setPadding(padding, padding, padding, padding);
				}
				if (MyDebug.LOG)
					Log.d(TAG,
							"addButtonOptionsToPopup time 2.2: "
									+ (System.currentTimeMillis() - time_s));
				...
				int total_width = 280;
				{
					...
					// the height should limit the width, due to when held in
					// portrait
					float density = getResources().getDisplayMetrics().density;
					int dpHeight = (int) (outMetrics.heightPixels / density);
					if (MyDebug.LOG)
						Log.d(TAG, "dpHeight: " + dpHeight);
					dpHeight -= 50; // allow space for the icons at top/right of
									// screen
				...
				params.width = (int) ((total_width / supported_options.size())
						* scale + 0.5f); // convert dps to pixels
				params.height = (int) (50 * scale + 0.5f); // convert dps to
															// pixels
				...
				if (MyDebug.LOG)
					Log.d(TAG,
							"addButtonOptionsToPopup time 2.3: "
									+ (System.currentTimeMillis() - time_s));
				...
				if (MyDebug.LOG)
					Log.d(TAG,
							"addButtonOptionsToPopup time 2.4: "
									+ (System.currentTimeMillis() - time_s));
			}
			if (MyDebug.LOG)
				Log.d(TAG,
						"addButtonOptionsToPopup time 3: "
								+ (System.currentTimeMillis() - time_s));
			this.addView(ll2);
			if (MyDebug.LOG)
				Log.d(TAG,
						"addButtonOptionsToPopup time 4: "
								+ (System.currentTimeMillis() - time_s));
		}
	}

	private void addRadioOptionsToPopup(List<String> supported_options,
			final String title, final String preference_key,
			final String default_option, final String test_key) {
		if (MyDebug.LOG)
		...
		if (supported_options != null) {
			...
			SharedPreferences sharedPreferences = PreferenceManager
					.getDefaultSharedPreferences(main_activity);
			String current_option = sharedPreferences.getString(preference_key,
					default_option);
			for (final String supported_option : supported_options) {
				if (MyDebug.LOG)
					Log.d(TAG, "supported_option: " + supported_option);
				...
				button.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						if (MyDebug.LOG)
							Log.d(TAG, "clicked current_option: "
									+ supported_option);
						SharedPreferences sharedPreferences = PreferenceManager
								.getDefaultSharedPreferences(main_activity);
						SharedPreferences.Editor editor = sharedPreferences
						...
						main_activity.updateForSettings(title + ": "
								+ supported_option);
						main_activity.closePopup();
					}
				});
				this.popup_buttons.put(test_key + "_" + supported_option,
						button);
			}
			this.addView(rg);
		}
	...
	private void addArrayOptionsToPopup(final List<String> supported_options,
		...
		if (supported_options != null && current_index != -1) {
			...
			final TextView resolution_text_view = new TextView(
					this.getContext());
			...
			final int padding = (int) (0 * scale + 0.5f); // convert dps to
															// pixels
			prev_button.setPadding(padding, padding, padding, padding);
			ViewGroup.LayoutParams vg_params = prev_button.getLayoutParams();
			vg_params.width = (int) (60 * scale + 0.5f); // convert dps to
															// pixels
			vg_params.height = (int) (50 * scale + 0.5f); // convert dps to
															// pixels
			prev_button.setLayoutParams(vg_params);
			prev_button.setVisibility((current_index > 0) ? View.VISIBLE
					: View.INVISIBLE);
			...
			next_button
					.setVisibility((current_index < supported_options.size() - 1) ? View.VISIBLE
							: View.INVISIBLE);
			...
			prev_button.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (new_index != -1) {
						resolution_text_view.setText(supported_options
								.get(new_index));
						prev_button
								.setVisibility((new_index > 0) ? View.VISIBLE
										: View.INVISIBLE);
						next_button
								.setVisibility((new_index < supported_options
										.size() - 1) ? View.VISIBLE
										: View.INVISIBLE);
					}
				}

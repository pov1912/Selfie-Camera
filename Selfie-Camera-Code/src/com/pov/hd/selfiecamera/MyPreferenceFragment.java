...
public class MyPreferenceFragment extends PreferenceFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.getActivity().setTheme(R.style.PreferenceTheme); // needed to avoid
																// sub-screens
																// opening with
																// the black
																// background
		...
		final boolean supports_auto_stabilise = bundle
				.getBoolean("supports_auto_stabilise");
		if (MyDebug.LOG)
			Log.d(TAG, "supports_auto_stabilise: " + supports_auto_stabilise);

		/*
		 * if( !supports_auto_stabilise ) { Preference pref =
		 * findPreference("preference_auto_stabilise"); PreferenceGroup pg =
		 * (PreferenceGroup
		 * )this.findPreference("preference_category_camera_effects");
		 * pg.removePreference(pref); }
		 */

		// readFromBundle(bundle, "color_effects",
		// Preview.getColorEffectPreferenceKey(), Camera.Parameters.EFFECT_NONE,
		// "preference_category_camera_effects");
		// readFromBundle(bundle, "scene_modes",
		// Preview.getSceneModePreferenceKey(),
		// Camera.Parameters.SCENE_MODE_AUTO,
		// "preference_category_camera_effects");
		// readFromBundle(bundle, "white_balances",
		// Preview.getWhiteBalancePreferenceKey(),
		// Camera.Parameters.WHITE_BALANCE_AUTO,
		// "preference_category_camera_effects");
		// readFromBundle(bundle, "isos", Preview.getISOPreferenceKey(), "auto",
		// "preference_category_camera_effects");
		// readFromBundle(bundle, "exposures", "preference_exposure", "0",
		// "preference_category_camera_effects");

		final boolean supports_face_detection = bundle
				.getBoolean("supports_face_detection");
		if (MyDebug.LOG)
		...
		if (!supports_face_detection) {
			...
			PreferenceGroup pg = (PreferenceGroup) this
					.findPreference("preference_category_camera_effects");
			pg.removePreference(pref);
		}
		...
		if (widths != null && heights != null) {
			...
			PreferenceGroup pg = (PreferenceGroup) this
					.findPreference("preference_screen_camera_quality");
			pg.removePreference(pref);
		}
		...
		final String[] video_quality_string = bundle
				.getStringArray("video_quality_string");
		if (video_quality != null && video_quality_string != null) {
			...
			lp.setEntries(entries);
			lp.setEntryValues(values);
			String video_quality_preference_key = MainActivity
					.getVideoQualityPreferenceKey(cameraId);
			String video_quality_value = sharedPreferences.getString(
		...
		final boolean supports_force_video_4k = bundle
				.getBoolean("supports_force_video_4k");
		if (MyDebug.LOG)
			Log.d(TAG, "supports_force_video_4k: " + supports_force_video_4k);
		if (!supports_force_video_4k || video_quality == null
				|| video_quality_string == null) {
		...
		final boolean supports_video_stabilization = bundle
				.getBoolean("supports_video_stabilization");
		if (MyDebug.LOG)
		...
		final boolean can_disable_shutter_sound = bundle
				.getBoolean("can_disable_shutter_sound");
		if (MyDebug.LOG)
			Log.d(TAG, "can_disable_shutter_sound: "
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1
			...
			PreferenceGroup pg = (PreferenceGroup) this
					.findPreference("preference_screen_camera_controls_more");
			pg.removePreference(pref);
		}

		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
			// Some immersive modes require KITKAT - simpler to require Kitkat
			// for any of the menu options
			Preference pref = findPreference("preference_immersive_mode");
			PreferenceGroup pg = (PreferenceGroup) this
					.findPreference("preference_screen_gui");
			pg.removePreference(pref);
		}
				...
				public boolean onPreferenceClick(Preference arg0) {
					if (pref.getKey().equals("preference_online_help")) {
						...
						Intent browserIntent = new Intent(Intent.ACTION_VIEW,
								Uri.parse("http://opencamera.sourceforge.net/"));
		 ...
		 * (EditTextPreference)findPreference("preference_save_location");
		 * InputFilter filter = new InputFilter() { // whilst Android seems to
		 * allow any characters on internal memory, SD cards are typically
		 * formatted with FAT32 String disallowed = "|\\?*<\":>"; public
		 * CharSequence filter(CharSequence source, int start, int end, Spanned
		 * dest, int dstart, int dend) { for(int i=start;i<end;i++) { if(
		 * disallowed.indexOf( source.charAt(i) ) != -1 ) { return ""; } }
		 * return null; } }; edit.getEditText().setFilters(new
		 * InputFilter[]{filter}); }
		 */
		{
			...
			pref.setOnPreferenceClickListener(new OnPreferenceClickListener() {
				...
				public boolean onPreferenceClick(Preference arg0) {
					if (pref.getKey().equals("preference_donate")) {
						...
						/*
						 * Intent browserIntent = new Intent(Intent.ACTION_VIEW,
						 * Uri.parse(MainActivity.getDonateMarketLink())); try {
						 * startActivity(browserIntent); }
						 * catch(ActivityNotFoundException e) { // needed in
						 * case market:// not supported if( MyDebug.LOG )
						 * Log.d(TAG, "can't launch market:// intent");
						 * browserIntent = new Intent(Intent.ACTION_VIEW,
						 * Uri.parse(MainActivity.getDonateLink()));
						 * startActivity(browserIntent); }
						 */
						Intent browserIntent = new Intent(Intent.ACTION_VIEW,
								Uri.parse(MainActivity.getDonateLink()));
			...
			pref.setOnPreferenceClickListener(new OnPreferenceClickListener() {
				...
				public boolean onPreferenceClick(Preference arg0) {
					if (pref.getKey().equals("preference_about")) {
						...
						int version_code = -1;
						try {
							PackageInfo pInfo = MyPreferenceFragment.this
									.getActivity()
									.getPackageManager()
									.getPackageInfo(
						...
						about_string.append("\nPackage: ");
						about_string.append(MyPreferenceFragment.this
						...
						about_string.append(Build.DEVICE);
						{
							...
							about_string.append(activityManager
									.getMemoryClass());
							about_string.append("\nLarge max heap? (MB): ");
							about_string.append(activityManager
									.getLargeMemoryClass());
						}
						{
							...
							Display display = MyPreferenceFragment.this
						...
						{
							SharedPreferences sharedPreferences = PreferenceManager
									.getDefaultSharedPreferences(MyPreferenceFragment.this
											.getActivity());
							String last_video_error = sharedPreferences
									.getString("last_video_error", "");
							if (last_video_error != null
									&& last_video_error.length() > 0) {
								about_string.append("\nLast video error: ");
						...
						about_string.append("\nAuto-stabilise?: ");
						about_string
								.append(getString(supports_auto_stabilise ? R.string.about_available
										: R.string.about_not_available));
						about_string.append("\nFace detection?: ");
						about_string
								.append(getString(supports_face_detection ? R.string.about_available
										: R.string.about_not_available));
						about_string.append("\nVideo stabilization?: ");
						about_string
								.append(getString(supports_video_stabilization ? R.string.about_available
										: R.string.about_not_available));
						about_string.append("\nFlash modes: ");
						String[] flash_values = bundle
								.getStringArray("flash_values");
						...
						about_string.append("\nFocus modes: ");
						String[] focus_values = bundle
								.getStringArray("focus_values");
						...
						about_string.append("\nColor effects: ");
						String[] color_effects_values = bundle
								.getStringArray("color_effects");
						if (color_effects_values != null
								&& color_effects_values.length > 0) {
						...
						about_string.append("\nScene modes: ");
						String[] scene_modes_values = bundle
								.getStringArray("scene_modes");
						if (scene_modes_values != null
								&& scene_modes_values.length > 0) {
						...
						about_string.append("\nWhite balances: ");
						String[] white_balances_values = bundle
								.getStringArray("white_balances");
						if (white_balances_values != null
								&& white_balances_values.length > 0) {
						...
						String save_location = sharedPreferences.getString(
								MainActivity.getSaveLocationPreferenceKey(),
								"OpenCamera");
						about_string
								.append("\nSave Location: " + save_location);

						about_string.append("\nParameters: ");
						String parameters_string = bundle
								.getString("parameters_string");
	...
	/*
	 * private void readFromBundle(Bundle bundle, String intent_key, String
	 * preference_key, String default_value, String preference_category_key) {
	 * if( MyDebug.LOG ) { Log.d(TAG, "readFromBundle: " + intent_key); } String
	 * [] values = bundle.getStringArray(intent_key); if( values != null &&
	 * values.length > 0 ) { if( MyDebug.LOG ) { Log.d(TAG, intent_key +
	 * " values:"); for(int i=0;i<values.length;i++) { Log.d(TAG, values[i]); }
	 * } ListPreference lp = (ListPreference)findPreference(preference_key);
	 * lp.setEntries(values); lp.setEntryValues(values); SharedPreferences
	 * sharedPreferences =
	 * PreferenceManager.getDefaultSharedPreferences(this.getActivity()); String
	 * value = sharedPreferences.getString(preference_key, default_value); if(
	 * MyDebug.LOG ) Log.d(TAG, "    value: " + values); lp.setValue(value); }
	 * else { if( MyDebug.LOG ) Log.d(TAG, "remove preference " + preference_key
	 * + " from category " + preference_category_key); Preference pref =
	 * findPreference(preference_key); PreferenceGroup pg =
	 * (PreferenceGroup)this.findPreference(preference_category_key);
	 * pg.removePreference(pref); } }
	 */
